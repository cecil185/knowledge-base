This side-project is similar to NotebookLM in many ways but here's whats unique about it: the goal-driven pipeline upstream of Q&A. This system runs /digest: pull HN + curated sources → filter against a written goal.md → auto-ticket the matches → summarize → synthesize. That's an agentic content pipeline with a personal relevance model, not a Q&A tool.
...
Where to focus:
  ...
  - Retrieval experiments (Contextual Retrieval, hybrid, reranker) measured against a real corpus you curated. The corpus being personal and meaningful gives the experiments narrative weight.
